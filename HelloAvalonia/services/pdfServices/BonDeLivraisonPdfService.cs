...
public class BonDeLivraisonPdfService
{
    private Document CreateBonDeLivraisonDocument(BonDeLivraisonViewModel deliveryNote, MainWindowViewModel main)
    {
            ...
            container.Page(page =>
            {
                    ...
                    .Column(column =>
                    {
                        ...
                        column.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem(2).Column(col =>
                            {
                                ...
                                var companyName = ExtractCompanyName(deliveryNote.CompanyInfo) ?? main.CompanyName ?? "";
                                ...
                                .Column(col =>
                                {
                                    if (!string.IsNullOrEmpty(deliveryNote.DeliveryNoteNumber))
                                    {
                                        col.Item().Row(r =>
                                        {
                                            ...
                                            r.RelativeItem().Text(deliveryNote.DeliveryNoteNumber);
                                        });
                                    }
                                    ...
                                    col.Item().PaddingTop(4).Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(deliveryNote.DeliveryNoteDate.ToString("dd/MM/yyyy"));
                                    });

                                    if (!string.IsNullOrEmpty(deliveryNote.QuotationNumber))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(80).Text("N° Devis:").Bold();
                                            r.RelativeItem().Text(deliveryNote.QuotationNumber);
                                        });
                                    }
                                });
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(deliveryNote.ClientName ?? "");
                                    });

                                    if (!string.IsNullOrEmpty(deliveryNote.ClientIce))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(60).Text("ICE:").Bold();
                                            r.RelativeItem().Text(deliveryNote.ClientIce);
                                        });
                                    }
                                });
                        });
                        ...
                        if (deliveryNote.Items.Any())
                        {
                            column.Item()
                                ...
                                .Table(table =>
                                {
                                    ...
                                    // Summary row
                                    var totalQuantity = deliveryNote.Items.Sum(item => item.Quantity);
                                    var numberOfReferences = deliveryNote.Items.Count;
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.ConstantItem(100).AlignRight().Text($"{deliveryNote.TotalHT:F2} DH");
                                    });

                                    col.Item().PaddingTop(8).Row(r =>
                                    {
                                        r.RelativeItem().Text($"TVA ({deliveryNote.VatRate}%):").Bold();
                                        r.ConstantItem(100).AlignRight().Text($"{deliveryNote.VatAmount:F2} DH");
                                    });
                                    ...
                                    col.Item().PaddingTop(16)
                                        ...
                                        .Row(r =>
                                        {
                                            ...
                                            r.ConstantItem(100).AlignRight()
                                                .Text($"{deliveryNote.TotalTTC:F2} DH")
                                                .Bold()
                                                .FontSize(14)
                ...
                page.Footer()
                    ...
                    .Column(col =>
                    {
                        ...
                        var companyAddress = ExtractCompanyAddress(deliveryNote.CompanyInfo) ?? main.CompanyAddress ?? "";
    ...
    public async Task<string> GenerateBonDeLivraisonPdfAsync(BonDeLivraisonViewModel deliveryNote, MainWindowViewModel main, string outputPath)
    {
        return await Task.Run(() =>
        {
            try
            {
                var document = CreateBonDeLivraisonDocument(deliveryNote, main);
    ...
    public async Task ShowBonDeLivraisonPreviewAsync(BonDeLivraisonViewModel deliveryNote, MainWindowViewModel main)
    {
        await Task.Run(() =>
        {
            try
            {
                ...
                catch (Exception companionEx)
                {
                    ...
                    var tempPdfPath = Path.Combine(Path.GetTempPath(), $"BonDeLivraisonPreview_{Guid.NewGuid()}.pdf");
