...
{
    private Document CreateQuotationDocument(QuotationViewModel quotation, MainWindowViewModel main)
    {
            ...
            container.Page(page =>
            {
                    ...
                    .Column(column =>
                    {
                        ...
                        column.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem(2).Column(col =>
                            {
                                ...
                                var companyName = ExtractCompanyName(quotation.CompanyInfo) ?? main.CompanyName ?? "";
                                ...
                                .Column(col =>
                                {
                                    if (!string.IsNullOrEmpty(quotation.QuotationNumber))
                                    {
                                        col.Item().Row(r =>
                                        {
                                            ...
                                            r.RelativeItem().Text(quotation.QuotationNumber);
                                        });
                                    }
                                    ...
                                    col.Item().PaddingTop(4).Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(quotation.QuotationDate.ToString("dd/MM/yyyy"));
                                    });

                                    if (quotation.DeliveryDate.HasValue)
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(80).Text("Date de livraison:").Bold();
                                            r.RelativeItem().Text(quotation.DeliveryDate.Value.ToString("dd/MM/yyyy"));
                                        });
                                    }
                                });
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(quotation.ClientName ?? "");
                                    });
                                    ...
                                    if (!string.IsNullOrEmpty(quotation.ClientAddress))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(60).Text("Adresse:").Bold();
                                            r.RelativeItem().Text(quotation.ClientAddress);
                                        });
                                    }

                                    if (!string.IsNullOrEmpty(quotation.ClientSite))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(60).Text("Site:").Bold();
                                            r.RelativeItem().Text(quotation.ClientSite);
                                        });
                                    }
                                });
                        });
                        ...
                        if (quotation.Items.Any())
                        {
                            column.Item()
                                ...
                                .Table(table =>
                                {
                                    ...
                                    var numberOfReferences = quotation.Items.Count;
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.ConstantItem(100).AlignRight().Text($"{quotation.TotalHT:F2} DH");
                                    });

                                    col.Item().PaddingTop(8).Row(r =>
                                    {
                                        r.RelativeItem().Text($"TVA ({quotation.VatRate}%):").Bold();
                                        r.ConstantItem(100).AlignRight().Text($"{quotation.VatAmount:F2} DH");
                                    });
                        ...
                        if (quotation.Conditions.Any())
                        {
                            column.Item()
                                ...
                                .Column(conditionsCol =>
                                {
                                    ...
                                    foreach (var condition in quotation.Conditions)
                                    {
                                        conditionsCol.Item()
                                            .PaddingBottom(8)
                                            .Column(condCol =>
                                            {
                                                ...
                                                {
                                                    condCol.Item().Text(condition.Title)
                                                        .FontSize(11)
                                                        .Bold();
                                                }
                                                ...
                                                if (!string.IsNullOrEmpty(condition.Content))
                                                {
                                                    ...
                                                    var contentLines = condition.Content.Split('\n', StringSplitOptions.RemoveEmptyEntries);
                ...
                page.Footer()
                    ...
                    .Column(col =>
                    {
                        ...
                        var companyAddress = ExtractCompanyAddress(quotation.CompanyInfo) ?? main.CompanyAddress ?? "";
    ...
    public async Task<string> GenerateQuotationPdfAsync(QuotationViewModel quotation, MainWindowViewModel main, string outputPath)
    {
        return await Task.Run(() =>
        {
            try
            {
                var document = CreateQuotationDocument(quotation, main);
    ...
    public async Task ShowQuotationPreviewAsync(QuotationViewModel quotation, MainWindowViewModel main)
    {
        await Task.Run(() =>
        {
            try
            {
                ...
                catch (Exception companionEx)
                {
                    ...
                    var tempPdfPath = Path.Combine(Path.GetTempPath(), $"QuotationPreview_{Guid.NewGuid()}.pdf");
