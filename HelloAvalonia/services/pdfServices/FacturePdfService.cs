...
{
    private Document CreateFactureDocument(FactureViewModel invoice, MainWindowViewModel main)
    {
            ...
            container.Page(page =>
            {
                    ...
                    .Column(column =>
                    {
                        ...
                        column.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem(2).Column(col =>
                            {
                                ...
                                var companyName = ExtractCompanyName(invoice.CompanyInfo) ?? main.CompanyName ?? "";
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    ...
                                    col.Item().PaddingTop(4).Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(invoice.InvoiceDate.ToString("dd/MM/yyyy"));
                                    });

                                    if (!string.IsNullOrEmpty(invoice.DeliveryNoteNumber))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(80).Text("Bon de Livraison N°:").Bold();
                                            r.RelativeItem().Text(invoice.DeliveryNoteNumber);
                                        });
                                    }
                                    ...
                                    if (!string.IsNullOrEmpty(invoice.OrderNumber))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(80).Text("Bon de Commande N°:").Bold();
                                            r.RelativeItem().Text(invoice.OrderNumber);
                                        });
                                    }

                                    if (!string.IsNullOrEmpty(invoice.QuotationNumber))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(80).Text("N° Devis:").Bold();
                                            r.RelativeItem().Text(invoice.QuotationNumber);
                                        });
                                    }
                                });
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.RelativeItem().Text(invoice.ClientName ?? "");
                                    });

                                    if (!string.IsNullOrEmpty(invoice.ClientIce))
                                    {
                                        col.Item().PaddingTop(4).Row(r =>
                                        {
                                            r.ConstantItem(60).Text("ICE:").Bold();
                                            r.RelativeItem().Text(invoice.ClientIce);
                                        });
                                    }
                                });
                        });
                        ...
                        if (invoice.Items.Any())
                        {
                            column.Item()
                                ...
                                .Table(table =>
                                {
                                    ...
                                    // Summary row
                                    var totalQuantity = invoice.Items.Sum(item => item.Quantity);
                                    var numberOfReferences = invoice.Items.Count;
                            ...
                            row.RelativeItem()
                                ...
                                .Column(col =>
                                {
                                    col.Item().Row(r =>
                                    {
                                        ...
                                        r.ConstantItem(100).AlignRight().Text($"{invoice.TotalHT:F2} DH");
                                    });

                                    col.Item().PaddingTop(8).Row(r =>
                                    {
                                        r.RelativeItem().Text($"TVA ({invoice.VatRate}%):").Bold();
                                        r.ConstantItem(100).AlignRight().Text($"{invoice.VatAmount:F2} DH");
                                    });
                ...
                page.Footer()
                    ...
                    .Column(col =>
                    {
                        ...
                        var companyAddress = ExtractCompanyAddress(invoice.CompanyInfo) ?? main.CompanyAddress ?? "";
    ...
    public async Task<string> GenerateFacturePdfAsync(FactureViewModel invoice, MainWindowViewModel main, string outputPath)
    {
        return await Task.Run(() =>
        {
            try
            {
                var document = CreateFactureDocument(invoice, main);
    ...
    public async Task ShowFacturePreviewAsync(FactureViewModel invoice, MainWindowViewModel main)
    {
        await Task.Run(() =>
        {
            try
            {
                ...
                catch (Exception companionEx)
                {
                    ...
                    var tempPdfPath = Path.Combine(Path.GetTempPath(), $"FacturePreview_{Guid.NewGuid()}.pdf");
