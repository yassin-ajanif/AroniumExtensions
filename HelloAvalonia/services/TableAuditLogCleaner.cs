...
public class TableAuditLogCleaner : ITableAuditLogCleaner
{
    ...
    public async Task CleanAsync()
    {
        try
        {
            ...
            var deleted = await DeleteOlderRowsAsync(_databasePath, lastInjectedId);
            if (deleted >= 0)
    ...
    public static async Task<int> DeleteOlderRowsAsync(string databasePath, int lastInjectedId)
    {
        var connectionString = $"Data Source={databasePath};Mode=ReadWrite;Default Timeout=10;";
